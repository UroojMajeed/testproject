...
describe('sanitize', () => {
  ...
  it('strips dotted keys that could reach into subdocuments', () => {
    const req = run({ body: { 'user.tokenVersion': 99, name: 'ok' } });
    expect(req.body['user.tokenVersion']).toBeUndefined();
  ...
  it('leaves clean payloads exactly as they were', () => {
    const clean = { name: 'Urooj', tags: ['a', 'b'], nested: { n: 1 } };
    expect(run({ body: structuredClone(clean) }).body).toEqual(clean);
  });
