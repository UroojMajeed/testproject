...
describe('registration payload', () => {
  ...
  it('refuses unknown keys so privilege fields cannot be smuggled in', () => {
    const r = registerSchema.body.safeParse({
      ...base, password: 'correct-horse-battery-staple', role: 'owner', tokenVersion: 42,
    });
    expect(r.success).toBe(false);
  });

  it('requires a name', () => {
    const r = registerSchema.body.safeParse({ ...base, name: '   ', password: 'correct-horse-battery-staple' });
    expect(r.success).toBe(false);
  });
});
