...
describe('randomToken', () => {
  ...
  it('does not repeat across many draws', () => {
    expect(new Set(Array.from({ length: 2000 }, () => randomToken())).size).toBe(2000);
  });
...
describe('safeEqual', () => {
  it('matches identical values', () => expect(safeEqual('same', 'same')).toBe(true));
  it('rejects different values of equal length', () => expect(safeEqual('abcdef', 'abcdeg')).toBe(false));
  it('rejects different lengths without throwing', () => expect(safeEqual('a', 'much-longer')).toBe(false));
