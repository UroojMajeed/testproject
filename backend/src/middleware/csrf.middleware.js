 ...
 * current browser — but Lax alone is not a guarantee on older ones. This adds
 * an explicit Origin/Referer check on the cookie-authenticated routes.
 */
export function verifyOrigin(req, _res, next) {
  ...
  if (!origin) {
    // A same-origin fetch from a browser always sends Origin on POST; a missing
    // header means a non-browser client, which cannot carry the cookie anyway.
    return next();
  }
