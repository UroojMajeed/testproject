import { User, RefreshToken, Membership } from '../../models/index.js';
import { ApiError } from '../../utils/ApiError.js';
import { ERROR_CODES, USER_STATUS, MEMBERSHIP_STATUS } from '../../config/constants.js';
...
export async function register({ name, email, password, timezone }, ctx) {
  ...
  if (existing) {
    // An account already exists. Saying so is an enumeration leak, so the
    // controller returns the same shape it would for a fresh signup and the
    // real account is told by email instead.
...
export async function login({ email, password }, ctx) {
  ...
  if (user.status !== USER_STATUS.ACTIVE || !user.passwordHash) {
    await burnPasswordTime();
    throw ApiError.unauthenticated('Email or password is incorrect', ERROR_CODES.INVALID_CREDENTIALS);
  }
 ...
 * Presenting an already-revoked token means the value leaked, so the whole
 * family is revoked and every access token for that user is invalidated.
 */
...
/**
 * Always resolves, whether or not the address exists — the controller returns
 * an identical response either way so the endpoint cannot enumerate accounts.
 */
...
/** Workspaces the caller actually belongs to — used to build the session payload. */
export async function listMemberships(userId) {
  return Membership.find({ userId, status: MEMBERSHIP_STATUS.ACTIVE })
    .populate('workspaceId', 'name slug industry timezone currency onboarding buybackRate')
    .lean();
}
