...
const mongoUri = z
  ...
  .refine(
    ...
    'must start with mongodb:// or mongodb+srv:// — copy the driver connection string, not the browser URL',
  )
  .refine(
    (v) => !/[<>]/.test(v),
    'still contains a placeholder in angle brackets — replace <password> (and any others) with the real value',
  );

/**
 * Every environment variable the server needs, validated at boot. If one is
 * missing or malformed the process refuses to start — a loud failure at 09:00
 * beats a silent one at 03:00.
 */
export const envSchema = z.object({
  NODE_ENV: z.enum(['development', 'test', 'production']).default('development'),
  PORT: z.coerce.number().int().positive().default(5000),

  // z.string().url() alone accepts "localhost:3000", which parses as a URL with
  // the scheme "localhost:". CORS would then never match and nothing would say
  // why, so require a real http(s) origin.
  CLIENT_URL: z
    .string()
    .url()
    .refine((v) => /^https?:\/\//.test(v), 'must include the scheme, for example http://localhost:3000')
...
export function databaseNameWarning(uri = '') {
  ...
  return `MONGODB_URI names no database, so "${DEFAULT_DB_NAME}" will be used. `
    + 'To be explicit, add it before the "?" — for example .mongodb.net/reclaimos?retryWrites=true';
}
