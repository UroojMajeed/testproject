...
export function splitUri(uri) {
  ...
  return {
    present: true,
    scheme,
    userinfo: afterScheme.slice(0, at),
...
/**
 * Describes the credentials without revealing them.
 *
 * Length is the fastest way to tell a stale password from a current one — if
 * Atlas gave you 24 characters and this says 16, the value in .env is simply
 * the old one. The character check catches the other real cause: a password
 * containing reserved URL characters that were never percent-encoded.
 */
export function describeCredentials(uri) {
  ...
  return {
    ...
    // A / or ? inside the userinfo means the string cannot be parsed reliably
    // by anything, this tool included. Say so instead of guessing.
  ...
  if (msg.includes('bad auth') || code === 8000 || code === 18) {
    return {
      ...
      fix: [
        ...
        'A change can also take up to a minute to take effect — if you just saved',
        'it, wait and run this again.',
      ],
    };
  }
...
async function main() {
  ...
  if (creds.present) {
    ...
    if (creds.looksEncoded) notes.push('contains a %XX sequence — make sure it is not double-encoded');

    if (creds.ambiguous) {
      console.log('  password   CANNOT BE READ — it contains a / or ? that is not percent-encoded,');
      console.log('             which makes the whole connection string ambiguous. Nothing can parse');
      console.log('             it reliably, including Mongo. Encode it (/ is %2F, ? is %3F) or, far');
      console.log('             easier, let Atlas autogenerate an alphanumeric password.');
    } else {
